...
struct Post {
    ...
    Post(int u, int p, int time, int l) {
        uid = u;
        pid = p;
        timestamp = time;
        like = l;
...
{
    p[pID] = Post(uID,pID,timestamp,0);
    cnt++;
}
...
{
    list<Post> li;
    list<Post>::iterator iter;
    for(int i = cnt; i>0; i--) {
        if(f[uID][p[i].uid]) {
            if(timestamp - p[i].timestamp > 1000) { // 초과
                if(li.size()>= 10) {
                    break;
                } else {
                    li.push_back(p[i]);
                ...
                for (iter = li.begin(); iter != li.end(); iter++) {
                    if (p[i].like > iter->like) {
                        li.insert(iter, p[i]);
                        isInserted = true;
                        break;
                    } else if (p[i].like == iter->like && p[i].timestamp > iter->timestamp) {
                        li.insert(iter, p[i]);
                        isInserted = true;
                        break;
                    }
                }
                if(!isInserted && li.size()<10) {
                    li.push_back(p[i]);
                }
            }
        }
